...
class SearchViewController: UIViewController, UISearchBarDelegate, UITableViewDelegate, UITableViewDataSource {
	...
	@IBOutlet var searchReturns: UILabel!
	var size = CGRect()

	@IBOutlet var tableView: UITableView!
	@IBOutlet var searchSuggestionsTableView: UITableView!
	@IBOutlet var recentSearchesTableView: UITableView!
	@IBOutlet var mainTableView: UIView!
	@IBOutlet var suggestionsView: UIView!
	@IBOutlet var recentSearchesView: UIView!
	@IBOutlet var searchSuggestionsView: UIView!
	@IBOutlet var allChaptersChartsButton: UIButton!
	@IBOutlet var chaptersButton: UIButton!
	@IBOutlet var chartsButton: UIButton!
	...
	func showTableView() {
		...
		allChaptersChartsButton.addTarget(self, action: #selector(showAllChapters), for: .touchUpInside)
		chaptersButton.addTarget(self, action: #selector(showChaptersOnly), for: .touchUpInside)
		chartsButton.addTarget(self, action: #selector(showChartsOnly), for: .touchUpInside)
	}
...
//
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		if let webViewViewController = segue.destination as? WebViewViewController {
			...
			func getURL(from filePath: URL, fallbackFileName: String) -> URL {
				if FileManager.default.fileExists(atPath: filePath.path) {
					print("Loading from Documents: \(filePath)")
					return filePath
				} else {
					return Bundle.main.url(forResource: fallbackFileName, withExtension: "html")!
				}
			}
			...
			switch (showCharts, showChapters) {
				...
				case (_, true):
					...
					url = chapterCodes[subArrayPointer] == "15_appendix_district_tb_coordinators_(by_district)"
						? getURL(from: filePath, fallbackFileName: chapterCodes[subArrayPointer])
						: Bundle.main.url(forResource: chapterCodes[subArrayPointer], withExtension: "html")!
	...
	func addBoldText(fullString: NSString, boldPartsOfString: [NSString]) -> NSAttributedString {
